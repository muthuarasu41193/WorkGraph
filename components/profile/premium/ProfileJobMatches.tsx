...
export default function ProfileJobMatches({ jobs = [], liveListings = 0, feedKind = "demo" }: Props) {
  ...
  return (
    <ProfileCard id="matches" padding="lg">
      ...
      {list.length === 0 ? (
        <p className="text-body text-[var(--text-primary)]/85">
          {feedKind === "live" && liveListings > 0 ? (
            ...
            <>
              ...
              <code className="text-caption">WORKGRAPH_API_URL</code> for semantic matches.
            </>
          )}
        </p>
      ) : null}
          ...
          >
            ...
            <div className="mt-4 flex items-center gap-2 border-t border-border pt-4">
              {job.applyUrl ? (
                ...
                <ProfileButton variant="primary" className="flex-1 py-2 text-caption" icon={<Zap className="h-3.5 w-3.5" />}>
                  Easy apply
                </ProfileButton>
              )}
              <Button
